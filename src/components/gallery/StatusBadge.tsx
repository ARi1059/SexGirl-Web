 ...
 * - availableToday → 玫瑰粉实心点 + 同色标签（文字取 availableTodayText，缺省「今日可接单」）。
 * - 否则有 statusText → ink-subtle 点 + 同色标签（如「已约满」「需预约 3 天」）。
 * - 两者皆无 → 不渲染。
 * 列表卡与详情页共用。availableTodayText 由管理员自定义（开发计划 M6-9）。
 */
export function StatusBadge({
  ...
  const text = availableToday
    ? availableTodayText?.trim() || "今日可接单"
    : statusText?.trim();
  if (!text) return null;
